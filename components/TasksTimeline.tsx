...
export default function TasksTimeline() {
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-3 py-8" style={{ color: '#9A8F83' }}>
          ...
          <div>
            {error === 'No HMRC connection' ? (
              <>
                ...
                <p style={{ color: '#7F1D1D' }}>请先完成 Self Assessment 的前两个步骤。</p>
              </>
            ) : error === 'NINO_MISMATCH' ? (
              <>
                <p className="font-semibold mb-1" style={{ color: '#991B1B' }}>NINO 与 HMRC 登录账号不匹配</p>
                <p className="mb-1" style={{ color: '#7F1D1D', lineHeight: 1.6 }}>
                  你在 Step 1 填写的 NINO，与连接 HMRC 时使用的 Government Gateway 账号不属于同一个用户。请确认两者一致，然后重新连接。
                </p>
              </>
            ) : (
              <>
